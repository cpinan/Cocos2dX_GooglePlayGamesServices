...
class GameHelperUtils {
	...
	static void printMisconfiguredDebugInfo(Context ctx) {
		...
		if (ctx == null) {
			Log.w("GameHelper",
					"*** (no Context, so can't print more debug info)");
			return;
		}
		...
		Log.w("GameHelper",
				"**** Package name         : " + ctx.getPackageName());
		Log.w("GameHelper", "**** Cert SHA1 fingerprint: "
				+ getSHA1CertFingerprint(ctx));
		Log.w("GameHelper", "**** App ID from          : "
				+ getAppIdFromResource(ctx));
		Log.w("GameHelper", "****");
		Log.w("GameHelper",
	...
	static String getString(Context ctx, int whichString) {
		...
		try {
			...
			ex.printStackTrace();
			Log.w(GameHelper.TAG,
					"*** GameHelper could not found resource id #"
							+ resId
							+ ". "
